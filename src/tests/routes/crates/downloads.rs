...
pub async fn assert_dl_count(
    ...
    let url = format!("https://crates.io/api/v1/crates/{name_and_version}/downloads");
...
pub async fn download(client: &impl RequestHelper, name_and_version: &str) {
    let url = format!("https://crates.io/api/v1/crates/{name_and_version}/download");
...
async fn test_crate_downloads() {
    ...
    let response = anon.get::<()>("https://crates.io/api/v1/crates/foo/downloads").await;
    ...
    let response = anon.get::<()>("https://crates.io/api/v1/crates/bar/downloads").await;
...
async fn test_version_downloads() {
    ...
    let response = anon.get::<()>("https://crates.io/api/v1/crates/foo/1.0.0/downloads").await;
    assert_eq!(response.status(), StatusCode::OK);
    let json = response.json();
    assert_json_snapshot!(json, {
        ".version_downloads[].date" => "[date]",
    ...
    // check different crate name
    let response = anon.get::<()>("https://crates.io/api/v1/crates/bar/1.0.0/downloads").await;
    assert_eq!(response.status(), StatusCode::NOT_FOUND);
    assert_snapshot!(
        response.text(),
        @r###"{"errors":[{"detail":"crate `bar` does not exist"}]}"###
    ...
    let response = anon.get::<()>("https://crates.io/api/v1/crates/foo/2.0.0/downloads").await;
    assert_eq!(response.status(), StatusCode::NOT_FOUND);
    assert_snapshot!(
        response.text(),
    ...
    let response = anon
        .get::<()>("https://crates.io/api/v1/crates/foo/invalid-version/downloads")
        .await;
    assert_eq!(response.status(), StatusCode::NOT_FOUND);
    assert_snapshot!(
        response.text(),
