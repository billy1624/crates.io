...
async fn dependencies() {
    ...
    let deps: Deps = anon
        .get("https://crates.io/api/v1/crates/foo_deps/1.0.0/dependencies")
        .await
        .good();
    assert_eq!(deps.dependencies[0].crate_id, "bar_deps");

    let response = anon
        .get::<()>("https://crates.io/api/v1/crates/missing-crate/1.0.0/dependencies")
        .await;
    ...
    let response = anon
        .get::<()>("https://crates.io/api/v1/crates/foo_deps/1.0.2/dependencies")
        .await;
    assert_eq!(response.status(), StatusCode::NOT_FOUND);
    assert_eq!(
        response.json(),
