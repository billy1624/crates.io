...
async fn test_owner_change_with_invalid_json() {
    ...
    let response = user
        .delete_with_body::<()>("https://crates.io/api/v1/crates/foo/owners", input.as_bytes())
        .await;
...
async fn test_unknown_crate() {
    ...
    let response = user
        .delete_with_body::<()>("https://crates.io/api/v1/crates/unknown/owners", body)
        .await;
...
async fn test_unknown_user() {
    ...
    let response = cookie
        .delete_with_body::<()>("https://crates.io/api/v1/crates/foo/owners", body)
        .await;
    assert_eq!(response.status(), StatusCode::BAD_REQUEST);
