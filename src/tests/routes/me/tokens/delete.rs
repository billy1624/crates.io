...
async fn revoke_token_non_existing() {
    ...
    let _json: RevokedResponse = user.delete("https://crates.io/api/v1/me/tokens/5").await.good();
}
...
async fn revoke_token_doesnt_revoke_other_users_token() {
    ...
    let _json: RevokedResponse = user2
        .delete(&format!("https://crates.io/api/v1/me/tokens/{}", token.id))
        .await
        .good();
...
async fn revoke_token_success() {
    ...
    let _json: RevokedResponse = user
        .delete(&format!("https://crates.io/api/v1/me/tokens/{}", token.as_model().id))
        .await
        .good();
