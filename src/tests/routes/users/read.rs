...
async fn show() {
    ...
    let json: UserShowPublicResponse = anon.get("https://crates.io/api/v1/users/foo").await.good();
    assert_eq!(json.user.login, "foo");

    let json: UserShowPublicResponse = anon.get("https://crates.io/api/v1/users/bAr").await.good();
...
async fn show_latest_user_case_insensitively() {
    ...
    let json: UserShowPublicResponse = anon.get("https://crates.io/api/v1/users/fOObAr").await.good();
    assert_eq!(
