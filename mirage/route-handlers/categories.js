...
export function register(server) {
  server.get('https://crates.io/api/v1/categories', function (schema, request) {
  ...
  server.get('https://crates.io/api/v1/categories/:category_id', function (schema, request) {
    let catId = request.params.category_id;
    let category = schema.categories.find(catId);
    return category ?? notFound();
  });

  server.get('https://crates.io/api/v1/category_slugs', function (schema) {
    let allCategories = schema.categories.all().sort((a, b) => compareStrings(a.category, b.category));
    return {
